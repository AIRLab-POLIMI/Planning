...
algorithms = ['voronoi_rrt_l1_1', 'voronoi_rrt_l1_5', 'voronoi_rrt_l1_10',
              'voronoi_rrt_l2_1', 'voronoi_rrt_l2_5', 'voronoi_rrt_l2_10',
              'theta_star_rrt']
ordering = 'theta_star_rrt'
...
def plot(m):
    wd = os.getcwd()
    df = pd.read_csv(wd + '/results/compare/' + m + '.csv')
    df = df[(df.algorithm == 'theta_star_rrt') | (df.algorithm == 'voronoi_rrt_l2_1') | (df.algorithm == 'voronoi_rrt_l2_10') |(df.algorithm == 'voronoi_rrt_l2_5')]
    #df2 = pd.read_csv(wd + '/results/compare/' + m + '_l2.csv')

    params = {'axes.labelsize': 10,'axes.titlesize': 10, 'text.fontsize': 10, 'legend.fontsize': 10, 'xtick.labelsize': 10, 'ytick.labelsize': 10}
    matplotlib.rcParams.update(params)

    fig, axes = plt.subplots(3, 1)
    sns.factorplot(x='conf', y ='time', hue='algorithm', data=df, ax= axes[0])
    #sns.factorplot(x='conf', y ='time', hue='algorithm', data=df2, ax= axes[0][1])

    sns.factorplot(x='conf', y ='length', hue='algorithm', data=df, ax= axes[1])
    #sns.factorplot(x='conf', y ='length', hue='algorithm', data=df2, ax= axes[1][1])

    sns.factorplot(x='conf', y ='roughness', hue='algorithm', data=df, ax= axes[2])
    #sns.factorplot(x='conf', y ='roughness', hue='algorithm', data=df2, ax= axes[2][1])
