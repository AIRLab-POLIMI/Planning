...
def draw(m, c):
    ...
    for a in algorithms:
        path = get_path(m, a, c)
        plt.plot(path[1:,0], path[1:,1], color=palette[names[a]], label=names[a], linewidth=2, zorder=1)

    plt.legend(fontsize=8)
    #plt.show()
    plt.axis('off')
    plt.savefig(os.getcwd() + '/paths/' + m + '_' + c + '.pdf')
    #plt.savefig(path + m + '_' + c + '.png')
    print 'wtf'
    plt.clf()
    plt.close()
...
def get_path(m, a, c):
    ...
    for i in range(0,50):
        if a == 'rrt_star_last':
            name = '/logs/' + m + '/'+ 'rrt_star' + '_' + m + '_' + c + '_'+ str(i) + '_last.log'
        else:
...
def draw_one(m, c, i):
    ...
    log = open(os.getcwd() + '/logs/'+ m + 'rrt_' + m + '_'+ c +'_'+ i + '.log', 'r')
    log_lines = log.readlines()
    np.set_printoptions(suppress=True)
    lines = log_lines[5:]
    start = log_lines[4].split('_')
