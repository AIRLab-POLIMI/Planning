...
def plot(m):
    ...
    for i in range(0,10):
        df_i = df_a[df_a.conf == i]
        plt.xlabel('Algorithm')
        plt.ylabel('Time (s)')
        time = sns.boxplot(x = "conf", y = "time", hue='algorithm', data = df_i, showfliers = False, ax = axes[0][i])

        plt.xlabel('Algorithm')
        plt.ylabel('Length')
        length = sns.boxplot(x = "conf", y = "length", hue='algorithm', data = df_i, showfliers = False, ax = axes[1][i])

        plt.xlabel('Algorithm')
        plt.ylabel('Roughness')
        roughness = sns.boxplot(x = "conf", y = "roughness", hue='algorithm', data = df_i, showfliers = False, ax = axes[2][i])
    ...
    axes[0][0].set_title("Time (s)")
    axes[1][0].set_title("Lenght")
    axes[2][0].set_title("Roughness")
